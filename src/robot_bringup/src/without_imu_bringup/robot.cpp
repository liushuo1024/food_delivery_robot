#include <vector>
#include "without_imu_bringup/robot.h"
#include "without_imu_bringup/mbot_linux_serial.h"
using namespace std;
...
namespace robot
{
    ...
    boost::array<double, 36> odom_twist_covariance = {
        ...
        0, 0, 0, 0, 1e6, 0, 
        0, 0, 0, 0, 0, 1e-9}};
    robot::robot():x_(0.0), y_(0.0), th_(0.0),vx_(0.0), vy_(0.0), vth_(0.0),sensFlag_(0),receFlag_(0) {
    ...
    ********************************************************/
    bool robot::init()
    {
        ...
        sub = nh.subscribe("/imu_data", 10, &robot::imu_data_CB, this);
        return true;
    }
    ...
    ********************************************************/
    ...
    void robot::imu_data_CB(const sensor_msgs::Imu &msg)
    {
        imu_vth_ = msg.angular_velocity.z;
        ROS_INFO("msg.angular_velocity.x :%f", msg.angular_velocity.x);
        ROS_INFO("msg.angular_velocity.y :%f", msg.angular_velocity.y);
        ROS_INFO("msg.angular_velocity.z :%f", msg.angular_velocity.z);
    }
}
